...
export const AgingReceivableView: React.FC = () => {
  ...
  const loadReport = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/reports/aging-receivable?date=${date}`);
      if (res.ok) {
        const result = await res.json();
        setData(result);
      }
    } catch (error) {
      console.error('❌ فشل جلب تقرير المتأخر المدين:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 animate-fade-in text-right" dir="rtl">
      ...
      <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-6 rounded-2xl border transition-all duration-300 ${
        ...
        <div>
          <div className="flex items-center gap-2">
            <Coins className={`${isNeon ? 'text-purple-400' : 'text-indigo-600'} h-6 w-6`} />
            <h1 className={`text-xl font-bold ${isNeon ? 'text-white' : 'text-slate-800'}`}>تقرير أعمار الذمم المدينة (AR Aging Report)</h1>
          </div>
          <p className={`text-xs mt-1 ${isNeon ? 'text-purple-300' : 'text-slate-500'}`}>
            تصنيف ومراقبة المستحقات على العملاء وحساب عمر الدين للمساعدة في متابعة التحصيل وتقليل الديون المعدومة.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button 
      ...
      <div className={`flex flex-wrap gap-4 items-end p-5 rounded-2xl border transition-all duration-300 ${
        ...
        <div className="w-full sm:w-auto flex-1 min-w-[200px]">
          <label className={`block text-xs font-bold mb-1.5 ${isNeon ? 'text-purple-300' : 'text-slate-500'}`}>البحث عن عميل</label>
      ...
      {loading ? (
        <div className={`p-12 text-center flex flex-col justify-center items-center gap-2 rounded-2xl border transition-all ${
          ...
          <p className="text-xs">جاري تحليل الذمم المدينة وتوزيع الأعمار...</p>
        </div>
      ) : data ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-7 gap-4">
            <div className={`p-4 rounded-2xl border shadow-sm flex flex-col justify-between ${
              ...
              <span className={`text-[10px] font-bold ${isNeon ? 'text-purple-300' : 'text-slate-500'}`}>العملاء المدينون</span>
            ...
            <div className={`p-4 rounded-2xl border shadow-sm flex flex-col justify-between ${
              ...
              <span className={`text-[10px] font-bold ${isNeon ? 'text-red-300' : 'text-red-700'}`}>إجمالي المديونية</span>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className={`border-b text-right font-bold ${
                    ...
                    <th className="p-4">إجمالي المديونية</th>
                    <th className="p-4">حالي (غير مستحق)</th>
                    <th className="p-4">1-30 يوم</th>
                    <th className="p-4">31-60 يوم</th>
                    <th className="p-4">61-90 يوم</th>
                    <th className="p-4">أكثر من 90 يوم</th>
                    <th className="p-4 text-center">درجة الخطورة / الحالة</th>
                  </tr>
                </thead>
                  ...
                  {filteredCustomers.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="p-8 text-center text-slate-400">
                        لا توجد مديونيات تطابق بحثك أو التاريخ المحدد.
                      </td>
                    </tr>
                  ) : (
                    filteredCustomers.map((customer) => {
                      const overdue90 = customer.days_90_plus > 0;
                      const overdue60 = customer.days_61_90 > 0;
                      const overdue30 = customer.days_31_60 > 0;
                      const overdue1 = customer.days_1_30 > 0;
                      ...
                      return (
                        <tr key={customer.id} className={`hover:bg-slate-50/50 transition-colors ${
                          ...
                          <td className="p-4">
                            <div>
                              <p className={`font-bold ${isNeon ? 'text-white' : 'text-slate-800'}`}>{customer.name}</p>
                              <p className="text-[10px] text-slate-400 font-mono mt-0.5">{customer.phone || customer.email || '-'}</p>
                            </div>
                          </td>
                          ...
                          <td className="p-4 font-bold text-red-600 font-mono">
                            {customer.days_90_plus > 0 ? formatCurrency(customer.days_90_plus) : '-'}
                          </td>
                            ...
                            <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black tracking-tight border ${
                              ...
                              {overdue90 ? (
                                <>
                                  ...
                                  <span>حرجة (متأخرة جداً)</span>
                                </>
                              ) : overdue60 ? (
                                <>
                                  <AlertTriangle className="h-3 w-3" />
                                  <span>عالية الخطورة</span>
                                </>
                              ) : overdue30 ? (
                                <>
                                  <span>متوسطة الخطورة</span>
                                </>
                              ) : overdue1 ? (
                                <>
                                  <span>منخفضة الخطورة</span>
                                </>
                              ) : (
                                <>
                                  <CheckCircle className="h-3 w-3" />
                                  <span>سليمة (حالية)</span>
                                </>
                              )}
                            </span>
                          </td>
        ...
        <div className="p-12 text-center text-slate-500 bg-white rounded-2xl border border-slate-200">
          لا توجد مديونيات أو عملاء تحت الطلب للفترة المحددة.
        </div>
      )}
    </div>
  );
...
export default AgingReceivableView;
